...
# ─── KONFIGURASI (WAJIB SAMA dengan tx_abp.py & rx_abp_ppg.py) ─────
FS               = 125
...
PAKET_PER_SEGMEN = 3750          # HARUS SAMA di tx_abp.py & rx_abp_ppg.py
STARTUP_GRACE_S  = 8.0           # jeda awal agar tx_abp.py & tx_ppg.py bisa mulai berdekatan
# ─────────────────────────────────────────────────────────────

# ─── KONFIGURASI KHUSUS FILE INI ────────────────────────────────
PORT           = 'COM12'         
BAUD_RATE      = 115200
OUTPUT_DIR     = 'hasil_tx_ppg'
WINDOW_S       = 10
THROUGHPUT_WIN_S    = 3
TX_PRINT_INTERVAL_S = 1.0
# ─────────────────────────────────────────────────────────────

SLOT_DURATION_S = PAKET_PER_SEGMEN / FS  
N_SEG = len(SEGMENTS)
...
def slot_mulai_ke(seg_i):
    # Urutan slot: ABP0, PPG0, ABP1, PPG1, ... → slot PPG segmen ke-i ada di indeks 2*i+1
    return SLOT_DURATION_S * (2 * seg_i + 1)


def simpan_ringkasan(seq_terkirim, n_target, elapsed_total_s, waktu_aktif_total_s, snapshot):
    # Metrik berbasis DURASI SESI (termasuk waktu nunggu giliran slot ABP)
    ...
    # Metrik berbasis WAKTU AKTIF KIRIM SAJA (tanpa waktu nunggu slot ABP/jeda awal)
    ...
    ring_path = os.path.join(OUTPUT_PATH, 'ringkasan_tx_ppg.csv')
    with open(ring_path, 'w', newline='', encoding='utf-8') as f:
        ...
        w.writerow([])
        w.writerow(['--- metrik rata2 vs DURASI SESI (termasuk nunggu slot ABP) ---', ''])
    ...
    print(f"[TX-PPG] Ringkasan -> {ring_path}")
...
def thread_tx():
    try:
        ...
        time.sleep(1.5)
        print(f"[TX-PPG] Terhubung ke {PORT}")
    except Exception as e:
        print(f"[TX-PPG] ERROR: {e}")
        stop_evt.set()
        return

    csv_path = os.path.join(OUTPUT_PATH, 'tx_ppg.csv')
    f = open(csv_path, 'w', newline='', encoding='utf-8')
    w = csv.writer(f)
    w.writerow(['timestamp_s', 'segmen_idx', 'segmen_label', 'seq',
                'ppg_raw', 'ppg_val', 'bytes_tx', 'bytes_tx_total',
    ...
    print(f"[TX-PPG] Menunggu {STARTUP_GRACE_S:.0f}s jeda awal — "
          f"jalankan tx_abp.py sekarang kalau belum!")
    ...
    for i, seg in enumerate(SEGMENTS):
        ...
        with lock:
            ...
            stats['segmen_label']  = f"menunggu slot PPG ({seg['label']})"

        wait_s = target_start - time.perf_counter()
        if wait_s > 0:
            print(f"[TX-PPG] Menunggu slot PPG segmen '{seg['label']}' ({wait_s:.1f}s lagi)...")
            time.sleep(wait_s)
        else:
            print(f"[TX-PPG] PERINGATAN: telat {-wait_s:.1f}s masuk slot '{seg['label']}', mulai sekarang.")

        with lock:
            stats['menunggu_slot'] = False
            stats['segmen_label']  = f"MENGIRIM: PPG ({seg['label']})"

        sig = load_segment_row(PPG_PATH, seg['idx'])
        n   = len(sig)
        ...
        for k in range(PAKET_PER_SEGMEN):
            ...
            packet    = f"START|TYPE:PPG|PPG:{raw}|SEQ:{seq}|END\n"
            pkt_bytes = len(packet.encode('utf-8'))

            try:
                ser.write(packet.encode('utf-8'))
            except Exception as e:
                print(f"[TX-PPG] Write error: {e}"); break
            ...
            w.writerow([round(time.perf_counter() - t0, 3), seg['idx'], seg['label'], seq,
                        raw, round(val, 4), pkt_bytes, bytes_tx_total_snap,
                        round(bps_tx, 1), round(bps_tx / 1000.0, 3), round(pps_tx, 2)])

            now_t = time.perf_counter()
            if now_t - last_print_t >= TX_PRINT_INTERVAL_S:
                print(f"[TX-PPG] seg={seg['label']:<10} {k+1:>5}/{PAKET_PER_SEGMEN} | "
        ...
        f.flush()
        print(f"[TX-PPG] Segmen {seg['label']} selesai — {PAKET_PER_SEGMEN} paket.")
    ...
    print(f"[TX-PPG] Selesai — total {seq:,} paket. CSV -> {csv_path}")
    print(f"[TX-PPG] Waktu aktif kirim: {waktu_aktif_total_s:.1f}s dari total sesi {elapsed_total_s:.1f}s "
...
def run_dashboard():
    ...
    sup_title = fig.suptitle(f'PPG Monitor — TX SAJA  |  Port: {PORT}',
                              color='#79C0FF', fontsize=13, fontweight='bold')
    ...
    ax1.set_title('Sinyal PPG - TX (yang dikirim)', color='#79C0FF', fontsize=10)
    ...
    line_tx, = ax1.plot(t_axis, list(buf_tx), color='#79C0FF', lw=1.0)
...
def main():
    n_target_total = PAKET_PER_SEGMEN * N_SEG
    print('=' * 70)
    print('  TX PPG SAJA — TDM berjadwal (proses terpisah dari TX ABP & RX)')
    print('=' * 70)
    print(f'  Port          : {PORT}')
    ...
    print('=' * 70)
    print('\nPENTING: jalankan tx_abp.py juga, sedekat mungkin waktunya dengan script ini!')
